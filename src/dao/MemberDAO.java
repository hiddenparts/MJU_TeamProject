...
public class MemberDAO {
	...
	public static boolean update(Member user) throws SQLException, NamingException {
		...
		try {
			...
			// 사진 변경이 없으면..
			if(user.getProfilephoto() == null) {// 질의 준비
				stmt = conn.prepareStatement(
						"UPDATE member " +
						"SET  userpassword=?, lastname=?, firstname=?, nickname=?, gender=?, email=?, introduce=?, website=?, info=?" +
						"WHERE userid=?"
						);
				...
				stmt = conn.prepareStatement(
						"UPDATE member " +
						"SET  userpassword=?, lastname=?, firstname=?, nickname=?, profilephoto=?, gender=?, email=?, introduce=?, website=?, info=?" +
						"WHERE userid=?"
						);
				stmt.setString(1,  user.getPwd());
				stmt.setString(2,  user.getLastname());
